#----------------------------------------------------------------
# This function performs multi-environment analysis for Diallel II experiments using Griffing Method                                   
# (Parents + F1's)                                    
# 
# ARGUMENTS:
...
diallel2TestME <-function(design = c("CRD", "RCB", "Alpha", "RowColumn"), data, respvar, p1, p2, rep=NULL, block=NULL, row=NULL, column=NULL, cross=TRUE, individual=NULL, environment, alpha=0.05) UseMethod("diallel2TestME")

diallel2TestME.default <-function(design = c("CRD", "RCB", "Alpha", "RowColumn"), data, respvar, p1, p2, rep=NULL, block=NULL, row=NULL, column=NULL, cross=TRUE, individual=NULL, environment, alpha=0.05) {
     ...
     #--- CREATION OF Matrices for GCA, SCA ---#
     ...
     for (z in (1:length(respvar))) {
          ...
          if ((nrow(temp.data)/nrow(data)) < 0.80) {
               ...
               if (nBalance<nrow(temp.data)) {
                    ...
                    if ((nrow(temp.data)/nBalance) >= 0.90) {
                         ...
                         for (x in (1:e)) {
                              if (design == "CRD") {
                                   ...
                                   outRecode<-recodeDiallelData(design="diallel2", data=tempDataForAnova, p1=p1, p2=p2, rep="Rep", respvar=respvar[z])
                              ...
                              if (design == "RCB") {
                                   ...
                                   # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
                                   outRecode<-recodeDiallelData(design="diallel2", data=tempDataForAnova, p1=p1, p2=p2, rep=block, respvar=respvar[z])
                                   balancedData<-outRecode$tempData
                                   codingGuide<-outRecode$newCoding
                              ...
                              if (design == "Alpha") {
                                   ...
                                   if (nrow(tempSplit)==nBalance/e) {
                                        ...
                                        # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
                                        outRecode<-recodeDiallelData(design="diallel2", data=tempDataForAnova, p1=p1, p2=p2, rep=rep, block=block, respvar=respvar[z])
                              ...
                              if (design == "RowColumn") {
                                   ...
                                   if (nrow(tempSplit)==nBalance/e) {
                                        ...
                                        # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
                                        outRecode<-recodeDiallelData(design="diallel2", data=tempDataForAnova, p1=p1, p2=p2, rep=rep, block=NULL, row=row, column=column, respvar=respvar[z])
                                        tempDataForAnova<-outRecode$tempData
                                        codingGuide<-outRecode$newCoding
                         ...
                         if (canEstimateMissing) {
                              ...
                              if (design == "Alpha") {
                                   # --- step 1 ANOVA ---#
                                   myformula1 <- paste(respvar[z], " ~ ", environment, " + ", environment, ":", rep, " + ", environment, ":", rep, ":", block, sep = "")
                                   out1 <- anova(aov(formula(myformula1), data = dataForAnova))
                              ...
                              data.all.gca <- merge(meanData, as.data.frame(cbind(M2,GCA,SCA)), by.x=c("newCodeP1","newCodeP2"), by.y=c("i","j"))
                              ...
                              colnames(GCA.effect) <- colnames(mdata)
                              rownames(GCA.effect) <- "GCA"
                              GCA.effect2 <-format(GCA.effect, digits=4, nsmall=4)
                              ...
                              colnames(SCA.effect) <- rownames(SCA.effect) <- seq(1:p)
                              SCA.effect2 <-format(round(SCA.effect,4), digits=4, nsmall=4)
                              
                              # recode back to the user's notation
                              rownames(SCA.effect2) <- colnames(SCA.effect2) <- codingGuide$levelsParents[match(colnames(SCA.effect2), codingGuide$newCodeParents)]
                              
                              
                              all.effect <- format(rbind(SCA.effect2, GCA.effect2),justify="right")
                    ...
                    cat("\n")
               } ## of else (if (nBalance<nrow(temp.data)))
