#----------------------------------------------------------------
# This function performs multi-environment analysis for Diallel I experiments using Griffing Method                                   
...
# R Script Modified by: Nellwyn Sales and Guoyou Ye 
#----------------------------------------------------------------

diallel1TestME <-function(design = c("CRD", "RCB", "Alpha", "RowColumn"), data, respvar, p1, p2, rep=NULL, block=NULL, row=NULL, column=NULL, cross=TRUE, individual=NULL, environment, alpha=0.05) UseMethod("diallel1TestME")
  
diallel1TestME.default <-function(design = c("CRD", "RCB", "Alpha", "RowColumn"), data, respvar, p1, p2, rep=NULL, block=NULL, row=NULL, column=NULL, cross=TRUE, individual=NULL, environment, alpha=0.05) {
      ...
      if ((nrow(temp.data)/nBalance) >= 0.90) {
        ...
        for (x in (1:e)) {
          if (design == "CRD") {
            ...
            outRecode<-recodeDiallelData(design="diallel1", data=tempDataForAnova, p1=p1, p2=p2, rep="Rep", respvar=respvar[z])
          ...
          if (design == "RCB") {
            ...
            # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
            outRecode<-recodeDiallelData(design="diallel1", data=tempDataForAnova, p1=p1, p2=p2, rep=block, respvar=respvar[z])
            balancedData<-outRecode$tempData
            codingGuide<-outRecode$newCoding
          ...
          if (design == "Alpha") {
            ...
            if (nrow(tempSplit)==nBalance/e) {
              ...
              # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
              outRecode<-recodeDiallelData(design="diallel1", data=tempDataForAnova, p1=p1, p2=p2, rep=rep, block=block, respvar=respvar[z])
          ...
          if (design == "RowColumn") {
            ...
            if (nrow(tempSplit)==nBalance/e) {
              ...
              # --- call recodeDiallelData to recode p1 and p2 to standard notation and generate balancedData --- #
              outRecode<-recodeDiallelData(design="diallel1", data=tempDataForAnova, p1=p1, p2=p2, rep=rep, block=NULL, row=row, column=column, respvar=respvar[z])
              tempDataForAnova<-outRecode$tempData
              codingGuide<-outRecode$newCoding
        ...
        if (canEstimateMissing) {
          ...
          data.all.gca <- merge(meanData, as.data.frame(cbind(M1,GCA,SCA,REC)), by.x=c("newCodeP1","newCodeP2"), by.y=c("i","j"))
          ...
          # MAT
          data.all.gca <- merge(meanData, as.data.frame(cbind(M1,GCA,SCA,MAT,NONM)), by.x=c("newCodeP1","newCodeP2"), by.y=c("i","j"))
          #data.all.gca <- data.all.gca[,-c(match("i", names(data.all.gca)),match("j", names(data.all.gca)))]
          ...
          meandata <- summaryBy(formula(myformula4), FUN=mean, data=dataForAnova)
          ...
          for (I in 1:p)  {
            for (J in 1:p)   {
              mdata[I,J] <- meandata[(meandata[,match("newCodeP1", names(meandata))]==I & meandata[,match("newCodeP2", names(meandata))]==J),3]
            } 
          }
          ...
          rownames(SCA.REC.effect) <- colnames(SCA.REC.effect) <- seq(1:p)
          
          # recode back to the user's notation
          SCA.REC.effect2 <-format(round(SCA.REC.effect,4), digits=4, nsmall=4, scientific=FALSE)
          rownames(SCA.REC.effect2) <- colnames(SCA.REC.effect2) <- codingGuide$levelsParents[match(colnames(SCA.REC.effect2), codingGuide$newCodeParents)]
          colnames(SCA.REC.effect2) <- paste(p2,"=",colnames(SCA.REC.effect2), sep="")
          rownames(SCA.REC.effect2) <- paste(p1,"=",rownames(SCA.REC.effect2), sep="")
          SCA.REC.effect2 <- noquote(gsub(" 0.0000", "", SCA.REC.effect2))
