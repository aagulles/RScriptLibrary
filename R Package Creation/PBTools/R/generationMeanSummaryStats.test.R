...
generationMeanSummaryStats.test<-function(data, meanVar, weightsVar, stdDevVar, numObsVar, generationVar, usersNotationVector, generalNotationVector, alpha) UseMethod("generationMeanSummaryStats.test")

generationMeanSummaryStats.test.default<-function(data, meanVar, weightsVar, stdDevVar, numObsVar, generationVar, usersNotationVector, generalNotationVector, alpha) {
	
	#trim the strings
	meanVar <- trimStrings(meanVar)
	generationVar <- trimStrings(generationVar)
	usersNotationVector <- trimStrings(usersNotationVector)
	generalNotationVector <- trimStrings(generalNotationVector)
	if (!(is.null(weightsVar))) weightsVar <- trimStrings(weightsVar)
	if (!(is.null(stdDevVar))) stdDevVar <- trimStrings(stdDevVar)
	if (!(is.null(numObsVar))) numObsVar <- trimStrings(numObsVar)
