...
graph.mea2s.boxhist <- function(data, respvar, result, box = FALSE, hist = FALSE) {
    ...
    #create histogram of the raw data (1 file for each envt in each respvar)
  	for (i in (1:length(respvar))) {
        ...
        png(filename = histfile) # par(mfrow = n2mfrow(length(respvar)*nlevels(data[,match(env, names(data))])));
				xlabel = respvar[i]
   			hist(result[[i]]$data[,match(respvar[i], names(result[[i]]$data))], xlab = xlabel, main = paste("Histogram of ", respvar[i]));
  			dev.off()
  	}	
  }	
