...
randomizeLattice.default <- function(k, r) {
	...
	if (r < 1 || r > k + 1) { stop("The object 'r' should be between 1 and k + 1.") }
	...
	trmtRandomize <- sample(1:ntrmt, ntrmt, replace = FALSE)
	...
	if (r > 2) {
		...
		if (k %% 2 != 0) {
			...
			if (k == 8) {
				...
				if (r > 4) {
					for (i in (4:r)) {
						if (i != 9) { index <- c(1,4,6,8,3,7,2,5)
						} else { index <- c(1,6,8,7,4,5,3,2) }
						tempLayout[[i-1]] <- matrix(tempLayout[[i-1]][order(t(temp[,index]))], nrow = b, ncol = k, byrow = TRUE)
					}
				}
			} # end stmt -- if (k == 8)
	...
	for (i in (1:length(tempLayout))){
		...
		book <- rbind(book, data.frame(Rep = i, Block = rep(((b*i)-(b-1)):(b * i), each = k), Treatment = as.numeric(t(tempLayout[[i]]))))
	}
