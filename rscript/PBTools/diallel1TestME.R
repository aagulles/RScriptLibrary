...
function(design = c("CRD", "RCB"), data, respvar, p1, p2, rep=NULL, block=NULL, cross=TRUE, individual=NULL, environment=NULL, alpha=0.05) {
  ...
  cat("\nDIALLEL ANALYSIS: GRIFFING METHOD I IN ",design, " (", parentsType, ")\n", sep="")
    ...
    print(AOV)
		...
		# --- GCA, SCA, REC, GCAxE, SCAxE, RECxE; DF, Sums of squares and mean squares --- #
		
		esteffect <- as.data.frame(cbind(M1,GCA,SCA,REC))
		data.all <- merge(temp.data,esteffect, by.x=c(p1,p2), by.y=c("i","j"))
		
		if (design == "CRD") {myformula2 <- paste(respvar[z], " ~ ", environment, " + . - ", p1, " - ", p2, " - ", rep, " + (. - ", p1, " - ", p2, " - ", rep, "):",environment, sep = "") }
		...
		# --- MAT, NONM, MATxE, NONMxE; DF, Sums of squares and mean squares --- #
		
		esteffect <- as.data.frame(cbind(M1,GCA,SCA,MAT,NONM))
		data.all <- merge(temp.data,esteffect, by.x=c(p1,p2), by.y=c("i","j"))
		
		if (design == "CRD") {myformula3 <- paste(respvar[z], " ~ ", environment, " + . - ", p1, " - ", p2, " - ", rep, " + (. - ", p1, " - ", p2, " - ", rep, "):",environment, sep = "") }
    ...
    print(mdata_print)
		...
		SCA.REC.effect2 <-format(round(SCA.REC.effect, 4), nsmall=4)
		SCA.REC.effect2 <- gsub(" 0.0000", "", SCA.REC.effect2)
		rownames(SCA.REC.effect2) <- colnames(SCA.REC.effect2) <- levels(temp.data[,match(p1, names(temp.data))])
		all.effect <- noquote(format(rbind(levels(temp.data[,match(p1, names(temp.data))]),SCA.REC.effect2, GCA.effect2),justify="right"))
		colnames(all.effect) <-rep("",ncol(all.effect))
		...
		rownames(VAREST) <- c(' Gi', ' Sii', ' Sij', ' Rij', ' Gi-Gj',
				' Sii-Sjj', ' Sii-Sij', ' Sii-Sjk', ' Sij-Sik',
				' Sij-Skl', ' Rij-Rkl')
