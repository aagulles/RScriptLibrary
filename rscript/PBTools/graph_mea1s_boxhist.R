...
graph.mea1s.boxhist <- function(data, respvar, result, box = FALSE, hist = FALSE) {
  ...
  if (box == TRUE) {
    ...
    for (i in (1:length(respvar))) {
  		...
  		xlabel = respvar[i]
      boxplot((data[,match(respvar[i], names(data))]), data = data,xlab = xlabel);
  		dev.off()
  	}
  }
  ...
  if (hist == TRUE) {
    #create histogram of the raw data (1 file for each envt in each respvar)
  	for (i in (1:length(respvar))) {
		...
		xlabel = respvar[i]
		hist(result$output[[i]]$data[,match(respvar[i], names(result$output[[i]]$data))], xlab = xlabel, main = paste("Histogram of ", respvar[i]));
		dev.off()
  	}	
  }	
