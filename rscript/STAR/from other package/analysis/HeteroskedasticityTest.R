...
HeteroskedasticityTest.default <- function(data, var, grp, method = c("bartlett")) {
	...
	for (i in (1:length(method))) {
		for (j in (1:length(var))) {
			for (k in (1:length(grp))) {
				...
				if (method[i] == "levene") hovTable <- rbind(hovTable, data.frame(GRP = grp[k], VARIABLE = var[j], PROCEDURE = method[i], DF = result[1,1], TEST.STAT = result[1,2], PVALUE = result[1,3]))
				if (method[i] == "bartlett") hovTable <- rbind(hovTable, data.frame(GRP = grp[k], VARIABLE = var[j], PROCEDURE = method[i], DF = result[[2]], TEST.STAT = result[[1]], PVALUE = result[[3]]))
			}
		}
	}
	...
	if (!is.null(hovTable)) { 
		...
		colnames(hovTable) <- c("Grp", "Variable", "Method", "DF", "Statistics", "Prob")
		options(width = 5000)
		cat(toupper("Test for Homogeneity of Variances"),"\n")
