...
CompareMeans.default <- function(
	...
	if (procedure == "one") {
		...
		cat("ONE SAMPLE t-TEST, h0 = ", testVal,"\n", sep = "")
	...
	if (procedure == "paired") {
		...
		names(varDiff) <- varX
		varLabel <- trim.strings(strsplit(paste(varX, collapse = " * ", varY, sep = " - "), split = "\\*")[[1]])
		...
		cat("\n")
		cat("PAIRED SAMPLE t-TEST, h0 = ", testVal,"\n", sep = "")
		printDataFrame(ttestResult)
		cat("\n")
	...
	if (procedure == "independent") {
		...
		cat("TWO INDEPENDENT SAMPLE t-TEST, h0 = ", testVal,"\n", sep = "")
		printDataFrame(ttestResult)
