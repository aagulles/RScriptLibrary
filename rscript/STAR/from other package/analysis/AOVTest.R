	     ...
	     estimateData <- FALSE
          ...
          for (z in (1:nlevels(data[,match(set, names(data))]))) {
               ...
               if (designChoice != 1) {
                    ...
                    if (is.list(replications(formula, rawData))) {
                         if (max(replications(formula, rawData)[[1]]) != 1) {
                              cat("ERROR: Cannot perform ANOVA for balance data. No unique data can be generated for variable '", respvar[i],"'.\n\n")
                              next
                         }
               ...
               if (!is.null(outputPath)) {
                    if (addSet) { png(filename = paste(outputPath, design,"DiagPlot_", respvar[i], ".png", sep = ""))
                    } else { png(filename = paste(outputPath, design,"DiagPlot_", setLabel,"_", respvar[i], ".png", sep = "")) }
               ...
               if (!is.null(sigEffect)) { 
                    ...
                    rvWithSigEffect <- unique(c(rvWithSigEffect, respvar[i]))
               }
               ...
               if (!is.null(sigEffect)) {
                    if (!is.na(match(respvar[i], pwVar))) {
                         for (j in (1:length(sigEffect))) {
                              pairwiseComparison(tempAnova[[i]][[z]], design, trim.strings(sigEffect[j]), data = tempData, respvar[i], pwTest, siglevel = sig)
                         }     
                    } else {
                         for (j in (1:length(sigEffect))) {
                              pairwiseComparison(tempAnova[[i]][[z]], design, trim.strings(sigEffect[j]), data = tempData, respvar[i], pwTest = NULL, siglevel = sig)
                         }     
                    }
               }
