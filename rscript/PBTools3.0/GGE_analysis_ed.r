...
#
# Modifications made by: Violeta Bartolome and Nellwyn L. Sales 
#----------------------------------------------------------------

gge.analysis <- function (ENV, GEN, REP, Y, MSE = 0, number = TRUE, yVar = "Y1", f=0.5, graphSym=FALSE, graphEnv=FALSE, graphGeno=FALSE,...) UseMethod("gge.analysis")

gge.analysis.default <- function (ENV, GEN, REP, Y, MSE = 0, number = TRUE, yVar = "Y1", f=0.5, graphSym=FALSE, graphEnv=FALSE, graphGeno=FALSE,...) {
    ...
    if (f==0.5 && graphSym) {
      ...
      if (ncol(bplot_new)>3) {
        #PC1 vs PC2 biplot with spokes; no polygon
        ...
        png(filename = paste(getwd(),"/", forFilename, "_what-won-where_biplot_",yVar,".png",sep = ""))
        par(cex=0.8)
      ...
      #  par(cex=0.8)
      #  plot(bplot_new[,3],bplot_new[,5],cex=0.8, xlab = "PC 1", ylab = "PC 3", frame = TRUE, pch=Type.code,col="white", main=paste(forFilename, " Biplot for ", gsub("_means", "", yVar), sep=""))
      #  if (number == TRUE) {
      ...
      #  par(cex=0.8)
      #  plot(bplot_new[,4],bplot_new[,5],cex=0.8, xlab = "PC 2", ylab = "PC 3", frame = TRUE, pch=Type.code,col="white", main=paste(forFilename, " Biplot for ", gsub("_means", "", yVar), sep=""))
      #  if (number == TRUE) {
