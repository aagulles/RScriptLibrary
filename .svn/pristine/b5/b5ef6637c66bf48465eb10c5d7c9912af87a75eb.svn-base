...
pairwiseComparison <- function(aovTable, design, effect, data, respvar, pwTest = NULL, siglevel = 0.05) {
     	...
     	if (length(strsplit(effect, split = ":")[[1]]) == 1) {
      	if (is.null(pwTest)) { if (nlevels(data[,effect]) <= 5) { pwTestChoice <- "LSD" } else { pwTestChoice <- "HSD" }}
          	...
          	if (designChoice <= 3) { pairwiseAmong(data, respvar, typeTest = pwTestChoice, trmt = effect, dfError = aovTable[[1]][nrow(aovTable[[1]]),1], MSError = aovTable[[1]][nrow(aovTable[[1]]),3], siglevel) 
            	...
            	start <- 1
               	while (is.na(match(effect, trim.strings(rownames(aovTable[[start]][[1]])))) && start <= length(aovTable)) start <- start + 1
          	...
          	if (designChoice > 3) {
              	...
              	for (i in (1:length(tempFactor))) {
                    	...
                    	while(cont && index <= length(aovTable)){
                         	if (!is.na(match(tempFactor[i], trim.strings(rownames(aovTable[[index]][[1]]))))) { tempPos <- c(tempPos, index) }
                         	index <- index + 1
                    	}
               	}
               	df <- list()
               	...
               	if (length(unique(tempPos)) > 1) {
                         	...
                         	if (any(unique(tempPos[-I(j)]) > tempPos[j])) {
                              	...
                              	for (k in (1:length(unique(tempPos[-I(j)])))) {
                                   		...
                                   		if (condition) {
                                        		...
                                        		start <- 1
                                        		while(is.na(match(paste(tempEffect, collapse = ":", sep = ""), trim.strings(row.names(aovTable[[start]][[1]])))) && start <= length(aovTable)) start <- start + 1
                    	...
                    	start <- 1
                    	while (is.na(match(effect,trim.strings(row.names(aovTable[[start]][[1]])))) && start <= length(aovTable)) start <- start + 1
