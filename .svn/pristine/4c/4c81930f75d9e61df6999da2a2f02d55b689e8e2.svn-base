	...
	if (procedure == "paired") {
		for (i in (1:length(varX))) {
			for (j in (1:length(varY))) {
				tempResult <- wilcox.test(x = data[,varX[i]], y = data[,varY[j]], alternative = altHypo, mu = hypoValue, paired = TRUE, conf.int = TRUE, conf.level = confLevel)
				if (confInt) {
					tempTable <- rbind(tempTable, 
							 data.frame(Variable = paste(varX[i], "-", varY[j]), Statistics = tempResult$statistic,
									Prob = tempResult$p.value, LL_CI = tempResult$conf.int[1], 
			     						Median = tempResult$estimate, UL_CI = tempResult$conf.int[2]))
				} else {
					tempTable <- rbind(tempTable, 
