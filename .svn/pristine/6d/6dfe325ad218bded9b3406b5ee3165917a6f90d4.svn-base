	  ...
	  for (j in (1:envNumLevels)) {
	    ...
	    temp.data[,match(p1, names(temp.data))] <- factor(trim.strings(temp.data[,match(p1, names(temp.data))]))
			...
			if (design == "CRD") {
				...
				balancedData<-generateBalancedData(design="DIALLEL3", data=tempDataForAnova, respvar[i], p1, p2, rep)
			}
			if (design == "RCB") {
				tempDataForAnova<-temp.data[,c(p1, p2, block, respvar[i])]
				balancedData<-generateBalancedData(design="DIALLEL3", data=tempDataForAnova, respvar[i], p1, p2, block)
			}
