...
LinearRegressionAnalysis.default <- function(data, depVar, indepVar, constant = TRUE, 
	...
	factorVar <- NULL
     	...
     	if (statistics) {
      	for (i in (1:length(indepVar))) {
          	...
          	DescriptiveStatistics(data = data, var = c(depVar, numVar), statistics = c("nnmiss", "mean", "sd", "se.mean")) 
          	cat("\n")
	...
	for (i in (1:length(depVar))) {
		...
		colnames(modelSummary) <- c("Root MSE", paste(depVar[i], "Mean"), "Coeff Var", "R-Square", "Adj R-Sq")			
		cat("Model Summary:\n")
		...
		if (covMatrix) {
			cat("Coefficicent Variance-Covariance Matrix\n")
		...
		if (!is.null(heteroskedasticity)) {
			...
			if (!is.na(match("bpagan", heteroskedasticity))) {
				...
				heteroTable <- data.frame(Method = result[[3]], DF = result[[2]], Stat = names(result[[1]]), Value = result[[1]], pvalue = result[[4]])
			} 
			if (!is.na(match("gquandt", heteroskedasticity))) {
				...
				if (!is.null(heteroTable)) {
					...
					heteroTable <- rbind(heteroTable, data.frame(Method = result[[3]], DF1 = result[[2]][[1]], DF2 = result[[2]][[2]], Stat = names(result[[1]]), Value = result[[1]], pvalue = result[[4]]))
					printDataFrame(heteroTable)
				} else {
					heteroTable <- data.frame(Method = result[[3]], DF1 = result[[2]][[1]], DF2 = result[[2]][[2]], Stat = names(result[[1]]), Value = result[[1]], pvalue = result[[4]])
					printDataFrame(heteroTable)
				}
				cat("\n\n")
			}
		...
		tempDataY <- data.frame(rnamesY = as.numeric(names(myfit[[i]]$residuals)), tempResid = myfit[[i]]$fitted.values)
		saveTable <- merge(saveTable, tempDataY, by.x = "rnamesX", by.y = "rnamesY", all.x = TRUE)
