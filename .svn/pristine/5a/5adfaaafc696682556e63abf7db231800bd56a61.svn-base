...
designSplit.default <- function(main, sub, ssub = NULL, sssub = NULL, r = NULL, trial = 1, design = c('crd', 'rcbd', 'lsd'), display = TRUE, file = NULL) {
	...
	if (design == "crd")  { capture.output(book <- designCRD(main, r, trial, display = FALSE))	 }
	if (design == "rcbd") { capture.output(book <- designRCBD(main, r, trial, display = FALSE))	 }
	if (design == "lsd")  { capture.output(book <- designLSD(main, trial, display = FALSE))		 }
