...
# Crosstab Function: Functions to create contingency table, chi-square test of independence,
#              phi coefficient, cramers v and contingency coefficient 
# Created by: Alaine A. Gulles 10.24.2010 for International Rice Research Institute
# Modified by: Alaine A. Gulles 10.27.2011
# --------------------------------------------------------------------------------------

Crosstab <- function(data, rowVar, colVar, obsFreq = TRUE, expFreq = FALSE, stdresid = FALSE, totPercent = FALSE, rowPercent = FALSE, colPercent = FALSE, chisq = FALSE, phi = FALSE, cramersv = FALSE, contingency = FALSE) UseMethod("Crosstab")

Crosstab.default <- function(data, rowVar, colVar, obsFreq = TRUE, expFreq = FALSE, stdresid = FALSE, totPercent = FALSE, rowPercent = FALSE, colPercent = FALSE, chisq = FALSE, phi = FALSE, cramersv = FALSE, contingency = FALSE) {
	...
	show.stat <- NULL; RTHeading <- FALSE; CTHeading <- FALSE
	if (obsFreq)    { nobsFreq <- nchar("Obs Freq"); 				RTHeading <- TRUE; CTHeading <- TRUE; show.stat <- c(show.stat, "obsFreq") } 		else nobsFreq <- 0
	if (expFreq)    { nexpFreq <- nchar("Expected Freq"); 			RTHeading <- TRUE; CTHeading <- TRUE; show.stat <- c(show.stat, "expFreq") } 		else nexpFreq <- 0
	if (totPercent) { ntotPercent <- nchar("% of Total"); 			RTHeading <- TRUE; CTHeading <- TRUE; show.stat <- c(show.stat, "totPercent") } 	else ntotPercent <- 0
	if (rowPercent) { RTHeading <- TRUE; 			  show.stat <- c(show.stat, "rowPercent") } 	
	if (colPercent) { CTHeading <- TRUE;			  show.stat <- c(show.stat, "colPercent") } 	

	for (i in (1:length(rowVar))) {
		for (j in (1:length(colVar))) {
			if (rowVar[i] != colVar[j]) {
				crosstable <- table(tempData[,rowVar[i]], tempData[,colVar[j]])
				...
				if (all(dim(crosstable) == 2)) { result <- suppressWarnings(chisq.test(tempData[,rowVar[i]], tempData[,colVar[j]], correct = TRUE)) 
				} else { result <- suppressWarnings(chisq.test(tempData[,rowVar[i]], tempData[,colVar[j]], correct = FALSE)) }
				if (any(c(obsFreq, expFreq, totPercent, rowPercent, colPercent) == TRUE)) {
					...
					if (length(show.stat) > 1) { cat("Table of ", rowVar[i], " by ", colVar[j], "\n\n", sep = "") 
						...
						if (obsFreq) cat("Table of Observed Frequency: ", rowVar[i], " by ", colVar[j], "\n\n", sep = "")
						if (expFreq) cat("Table of Expected Frequency: ", rowVar[i], " by ", colVar[j], "\n\n", sep = "")
					...
					if (obsFreq || expFreq || totPercent || rowPercent) {
					...
					for (k in (1:(nrow(crosstable)+1))) {
						...
						if (expFreq) {
							if (k <= nrow(crosstable)) {
								if (show.stat[1] != "expFreq") { cat(formatC("", width = RowHeading1 + 1, format = "s"), "|", sep = "") }
						...
						} ### end stmt -- if (rowPercent && k <= nrow(crosstable))
				...
				if (chisq || phi || cramersv || contingency) {
					...
					if (chisq) {
						...
						tlabel.length2 <- max(nchar(result[[2]][[1]]),2) + 2
						tlabel.length3 <- nchar(round(result[[1]][[1]], 4)) + 2
					} else { tlabel.length2 <- 0; tlabel.length3 <- 0 }
					...
					tlabel.length1 <- max(nchar(chisq.label)) + 2
					...
					cat(formatC("Statistics", width = tlabel.length1, format = "s", flag = "-"), sep = "")
					if (chisq) cat(formatC("DF", width = tlabel.length2, format = "s", flag = "-"), sep = "")
	      	          	cat(formatC("Value", width = tlabel.length3, format = "s"), sep = "")
					if (chisq) cat(formatC("Prob", width = 9, format = "s"), sep = "")
					cat("\n")

					cat(formatC(paste(rep("-", tlabel.length1), collapse = ""), width = tlabel.length1), sep = "")
					if (chisq) cat(formatC(paste(rep("-", tlabel.length2), collapse = ""), width = tlabel.length2), sep = "")
					cat(formatC(paste(rep("-", tlabel.length3), collapse = ""), width = tlabel.length3), sep = "") 
					if (chisq) cat(formatC(paste(rep("-", 9), collapse = ""), width = 9), sep = "")
					cat("\n")

					if (chisq) {
						cat(formatC("Pearson Chi-Square", width = tlabel.length1, format = "s", flag = "-"), 
						    formatC(result[[2]][[1]], width = tlabel.length2, flag = "-"), 
    						    formatC(result[[1]][[1]], digits = 4, width = tlabel.length3, format = "f", flag = "#"), 
						    formatC(result[[3]][[1]], digits = 4, width = 8, format = "f", flag = "#"), "\n", sep = "")
						cat(formatC(chisq.label[2], width = tlabel.length1, format = "s", flag = "-"), 
						    formatC(LikelihoodRatioTest(crosstable)$df, width = tlabel.length2, flag = "-"), 
	    					    formatC(LikelihoodRatioTest(crosstable)$statistics, digits = 4, width = tlabel.length3, format = "f", flag = "#"), 
						    formatC(LikelihoodRatioTest(crosstable)$p.value, digits = 4, width = 8, format = "f", flag = "#"), "\n", sep = "")
					}

					if (phi) { 
						cat(formatC("Phi Coefficient", width = tlabel.length1, format = "s", flag = "-"), sep = "")
						if (chisq) cat(formatC("", width = tlabel.length2, format = "s", flag = "-"), sep = "")
					    	cat(formatC(PhiCoef(crosstable), digits = 4, width = tlabel.length3, format = "f", flag = "#"), "\n", sep = "")
					}
					if (contingency) {
						cat(formatC("Contingency Coefficient", width = tlabel.length1, format = "s", flag = "-"), sep = "")
						if (chisq) cat(formatC("", width = tlabel.length2, format = "s", flag = "-"), sep = "")
					    	cat(formatC(ContingencyCoef(crosstable), digits = 4, width = tlabel.length3, format = "f", flag = "#"), "\n", sep = "")
					}
					if (cramersv) {
						cat(formatC("Cramer's V", width = tlabel.length1, format = "s", flag = "-"), sep = "")
						if (chisq) cat(formatC("", width = tlabel.length2, format = "s", flag = "-"), sep = "")
					    	cat(formatC(CramersV(crosstable), digits = 4, width = tlabel.length3, format = "f", flag = "#"), "\n", sep = "")
					}
					if (all(dim(crosstable) == 2) && chisq) {
						cat(formatC("Fisher's Exact Test", width = tlabel.length1, format = "s", flag = "-"), 
						    formatC("", width = tlabel.length2, format = "s", flag = "-"), 
						    formatC("", width = tlabel.length3, format = "s", flag = "-"), 
					...
					cat("\n")
				} ### end stmt -- if (chisq || phi || cramersv || contingency)
