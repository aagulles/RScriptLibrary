...
CompareProportion.default <- function(data, varX, varY = NULL, 
	...
	if (procedure == "one") {
		...
		for (i in (1:length(varX))) {
			...
			if (length(tempTable) != 2) { stop(paste("The variable'",varX[i],"' should be a dichotomous variable.", sep = "")) 
			} else { 
				if (any(is.na(match(names(tempTable), c("0","1"))))) {
					stop(paste("The variable'",varX[i],"' should be a dichotomous variable with value 1 for success and 0 for failure.", sep = ""))
	...
	if (procedure == "independent") {
		...
		for (i in (1:length(varX))) {
			...
			if (nrow(tempTable) != 2) { stop(paste("The variable '",varX[i],"' should be a dichotomous variable.", sep = "")) 
			} else { 
	...
	if (procedure == "paired") {
		...
		summaryResult <- NULL
		for (i in (1:length(varX))) {
			if (length(unique(data[,varX[i]])) != 2 || length(unique(data[,varY[i]])) != 2) stop("The arguments 'varX' and 'varY' should both be dichotomous variables.")
