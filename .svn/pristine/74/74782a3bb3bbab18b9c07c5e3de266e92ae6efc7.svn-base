...
designAugmented.default <- function(checkTrmt, newTrmt, r = NULL, trial = 1, design = c("rcbd", "lsd"), factorName = NULL, display = TRUE, file = NULL) {
	...
	if (design == "lsd")  { 
		...
		check <- TRUE
		addItem <- 1
          	tempCntNewTrmt <- length(checkTrmt)*addItem + (addItem)**2
          	check <- TRUE
          	fewNewTrmt <- FALSE
          	while (check) {
               		if (tempCntNewTrmt > length(newTrmt)) { check <- FALSE; fewNewTrmt <- TRUE }
               		if (tempCntNewTrmt == length(newTrmt)) { check <- FALSE }
               		if (tempCntNewTrmt < length(newTrmt)) {
                    		addItem <- addItem + 1
                    		tempCntNewTrmt <- length(checkTrmt)*addItem + (addItem)**2
               		}
          	}
          
          	if (fewNewTrmt) { stop("There are too few new treatments.") }
