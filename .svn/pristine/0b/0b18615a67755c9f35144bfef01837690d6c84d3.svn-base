...
# Modified by: Alaine A. Gulles 07.30.2012
# -------------------------------------------------------------------------------
    ...
    if (!is.na(match(hEffect, rownames(tableSigEffect)))) { sigEffect <- hEffect 
      ...
      if (length(allFactor) > 1) { 
        ...
        if (numFactorCheck > 1) { checkOtherEffect <- TRUE 
          ...
          tableSigEffect <- tableSigEffect[-I(match(trim.strings(rownames(tableSigEffect))[-I(match(trim.strings(allFactor), trim.strings(rownames(tableSigEffect))))], trim.strings(rownames(tableSigEffect)))),]
        }
        # determine intial significant effect
        sigEffect <- c(sigEffect, trim.strings(rownames(subset(tableSigEffect, numOfEffect == numFactorCheck))))
        ...
        while (checkOtherEffect) {
          ...
          if (!is.null(factorToCheck)) {
            ...
            for (i in (1:length(factorToCheck))) {
              if (nrow(subset(tableSigEffect, trim.strings(rownames(tableSigEffect)) == factorToCheck[i])) != 0) {
                sigEffect <- c(sigEffect,trim.strings(rownames(subset(tableSigEffect, trim.strings(rownames(tableSigEffect)) == factorToCheck[i]))))
                newSigFactor <- TRUE
              }
            }
            if (newSigFactor) {
