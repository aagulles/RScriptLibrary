# --------------------------------------------------------
# RCropStat Beta Menu
# Functions obtained/copyied from other package
# Package MASS
# --------------------------------------------------------
# make.contrasts
...
make.contrasts <-  function (contr, how.many=ncol(contr)) {
  if(!is.matrix(contr)) contr <- matrix(contr,ncol=length(contr))
  if(nrow(contr)+1 > how.many) stop("Too many contrasts specified. Must be less than the number of factor levels (columns).")
  
  value <- as.matrix(ginv(contr))  # requires library(MASS)
  if (nrow(value) != how.many) stop("wrong number of contrast matrix rows")
