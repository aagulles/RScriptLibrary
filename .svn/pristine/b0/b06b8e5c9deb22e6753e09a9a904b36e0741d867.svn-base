...
designRCBD <- function(generate, r = 2, trial = 1, display = TRUE, file = NULL) UseMethod("designRCBD")

designRCBD.default <- function(generate, r = 2, trial = 1, display = TRUE, file = NULL) {
	...
	for (i in (1:trial)) {
		for (j in (1:r)) {
			temp <- data.frame(Trial = as.character(i), Block = as.character(j), tempComb, PlotNum = sample(nrow(tempComb), nrow(tempComb), replace = FALSE))
