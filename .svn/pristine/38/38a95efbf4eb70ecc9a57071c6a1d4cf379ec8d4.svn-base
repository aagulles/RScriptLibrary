...
graph.mea1s.diagplots <- function(data, respvar, is.random = FALSE, result) {
  ...
  for (i in (1:length(respvar))) {
	...
	cc = rgb(0.3, 0.3, 0.7, 0.2)
      ...
      #scatterplot of residuals against fitted values
  		plot(result$output[[i]]$residuals~result$output[[i]]$fitted.values, xlab = "Predicted Values", pch = 15, cex = 0.7, col = cc, ylab = "Residuals", main = xlabel);

      #qqplot of residuals
			qqnorm(result$output[[i]]$residuals); qqline(result$output[[i]]$residuals, col=2, main=title,sub=xlabel)
	
      #freq dist of residuals
			hist(result$output[[i]]$residuals, main = "Residuals",  col = cc, xlab = "Residual", ylab = 'Frequency' )
