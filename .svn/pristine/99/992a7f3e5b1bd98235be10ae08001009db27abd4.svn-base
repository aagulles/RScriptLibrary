...
ContrastCompute <- function(data, aovTable, mymodel, mymodel2, contrast.option) {
	...
	summary.aovcontrast <- eval(parse(text = paste("summary(result.aovcontrast, split = list(", paste(cm.label2, collapse = ",", sep = ""), "))", sep = "")))
     ...
     if (attr(summary.aovcontrast,"class")[[1]] == "summary.aovlist") {
          ...
          for(i in (2:length(summary.aovcontrast))) {
               for (j in (1:2)) {
                    if (j == 1) { condition <- trim.strings(rownames(summary.aovcontrast[[i]][[1]][is.na(summary.aovcontrast[[i]][[1]][,"Sum Sq"]),]), side = "right")
                    } else { condition <- trim.strings(rownames(summary.aovcontrast[[i]][[1]][summary.aovcontrast[[i]][[1]][,"Sum Sq"] == 0,]), side = "right") }
                    if (length(condition) > 0) {
                         index1 <- match(condition, trim.strings(rownames(summary.aovcontrast[[i]][[1]]), side = "right"))
                         index2 <- match(condition, trim.strings(rownames(tempSummary[[1]]), side = "right"))  
	...
	cat("ANOVA TABLE\nResponse Variable: ", trim.strings(strsplit(mymodel, split = "~")[[1]][1]), "\n\n", sep = "")
