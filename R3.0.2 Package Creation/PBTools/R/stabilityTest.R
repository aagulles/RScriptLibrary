...
# Created by: Nellwyn L. Sales for International Rice Research Institute
# Modified by: Alaine A. Gulles 02.26.2015
...
#
# FUNCTIONS NEEDED: trimStrings, reshape {stats}
#--------------------------------------------------------------------------------
...
stabilityTest.default <- function (data, outFileName, respvar, environment, genotype, useFinlay=TRUE, useShukla=FALSE) {
     ...
     # added by AAGulles 02.26.2015
     tempFile <- strsplit(outFileName, split = "/")[[1]]
     tempFile <- paste(tempFile[1:(length(tempFile)-1)], collapse = "/")
          ...
          if (nlevels(temp.data[,environment])>4) {
               ...
               if (responseRate < 0.80) {
                    ...
                    if (useFinlay) {
                         ...
                         suppressWarnings(capture.output(result.temp<-try(FinlayStability(data = temp.data, respvar = respvar[i], genotype, environment, outputPath = tempFile), silent=TRUE)))
                         ...
                         if (!is.null(class(result.temp)) & class(result.temp)=="try-error") {  
                              ...
                              msg <- gsub("\"", "", msg)
                              capture.output(cat("*** \nERROR in FinlayStability function:\n  ",msg, "\n***\n\n\n", sep = ""), file=outFileName,append = TRUE)
                              suppressWarnings(result$respvar[[i]]$Error<-paste("ERROR: ", msg, sep=""))
                         } else {
                              capture.output(cat("Analysis of Variance\n"),file=outFileName,append = TRUE)
                              capture.output(printAOVTable(result.temp[[1]]),file=outFileName,append = TRUE)
                              capture.output(cat("\n\n"),file=outFileName,append = TRUE)
                              if (!is.null(result.temp[[2]])) {
                                   capture.output(printDataFrame(result.temp[[2]], digits = 4),file=outFileName,append = TRUE)
                                   capture.output(cat("\n\n"),file=outFileName,append = TRUE)
                              } else {
                                   capture.output(cat("no variation in site means.\n\n"),file=outFileName,append = TRUE)
                              }
                              suppressWarnings(result$respvar[[i]]$Finlay<-result.temp)
                    ...
                    if (useShukla) {
                         ...
                         capture.output(result.temp2 <-try(ShuklaStability(data = temp.data, respvar = respvar[i], genotype, 
                                                                           environment, outputPath = tempFile),silent = TRUE))
                         
                         if (!is.null(class(result.temp2)) & class(result.temp2)=="try-error") {  
                              msg <- trimStrings(strsplit(result.temp2, ":")[[1]])
                              msg <- trimStrings(paste(strsplit(msg, "\n")[[length(msg)]], collapse = " "))
                              msg <- gsub("\"", "", msg)
                              capture.output(cat("*** \nERROR in ShuklaStability function:\n  ",msg, "\n***\n\n\n", sep = ""), file=outFileName,append = TRUE)
                              suppressWarnings(result$respvar[[i]]$Error<-paste("ERROR: ", msg, sep=""))
                         } else {
                              if (is.null(result.temp2$errormsg)) {
                                   capture.output(printDataFrame(result.temp2$stability, digits = 4), file = outFileName, append = TRUE)
                                   capture.output(cat("\n\n"),file = outFileName, append = TRUE)
                                   suppressWarnings(result$respvar[[i]]$Shukla <- result.temp2$stability)     
                              } else {
                                   capture.output(cat("*** \nERROR in ShuklaStability function:\n  ", result.temp2$errormsg, "\n***\n\n\n", sep = ""), file=outFileName,append = TRUE)
                                   suppressWarnings(result$respvar[[i]]$Error<-paste("ERROR: ", result.temp2$errormsg, sep=""))
                              }
                              
                         }
                    } 
               } ## end stmt -- if (responseRate < 0.80) else
