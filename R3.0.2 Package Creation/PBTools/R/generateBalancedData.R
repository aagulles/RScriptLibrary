#-------------------------------------------------
# This function generates balanced data for nested, factorial and diallel designs
#
# ARGUMENTS:
# design - design structure; values are "NESTED","FACTORIAL", "DIALLEL2", "DIALLEL3", "DIALLEL4"
# data - data frame
# respvar - string, response variable
...
generateBalancedData <- function(design=c("NESTED","FACTORIAL", "DIALLEL2", "DIALLEL3", "DIALLEL4"), data, respvar, male, female, rep) UseMethod("generateBalancedData")
  
generateBalancedData.default <- function(design=c("NESTED","FACTORIAL", "DIALLEL2", "DIALLEL3", "DIALLEL4"), data, respvar, male, female, rep) {
	tempData<-data
	...
	if (design == "NESTED")  {
      ...
      if (!any(is.na(as.numeric(levelsParent)))) {
        levelsParent<-levels(factor(as.numeric(levelsParent)))
      }
	...
	} else if (nrow(tempComplete)>nrow(fullTrtCombi)) {
	  tempData <- fullTrtCombi
	}  
	tempData$Cross <- eval(parse(text = paste("tempData[,'",male,"']:tempData[,'",female,"']", sep="")))
	return(tempData)
}
