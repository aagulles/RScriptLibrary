#-------------------------------------------------
# This function formats the numbers in the Anova Table to a specific format
#
# ARGUMENT:
# anova.table - can be summary.aov object, anova object, or data frame
# 
# Author: Nellwyn Sales
#-------------------------------------------------

formatAnovaTable <- function(anova.table) UseMethod("formatAnovaTable")

formatAnovaTable.default <- function(anova.table) {
  ...
  for (i in 1:length(Fcol)) {
    if (is.na(df[i, "F value"])) {
      ...
      Frow_f<-format(round(df[i, "F value"],2), digits=2, nsmall=2, scientific=FALSE)
    }
